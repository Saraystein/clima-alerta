document.addEventListener('DOMContentLoaded', function() {
  ...
  cardsDia.forEach(card => {
    card.addEventListener('click', function() {
      ...
      const partes = diaClicado.split('/');
      const dataAlvo = `2025-${partes[1]}-${partes[0]}`;

      const diaSelecionado = dados.filter(item => item.dt_txt.startsWith(dataAlvo));
      ...
      if (diaSelecionado.length > 0) {
        ...
        document.getElementById('descricao-principal').innerText = primeiro.weather[0].description.charAt(0).toUpperCase() + primeiro.weather[0].description.slice(1);
...
// Atualizar gráfico corretamente
